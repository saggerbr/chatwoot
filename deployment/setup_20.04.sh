...
function setup_chatwoot() {
  ...
  git clone https://github.com/saggerbr/chatwoot.git
  cd chatwoot
  git checkout "$BRANCH"
  chmod -R 777 bin
  bundle
...
function setup_ssl() {
  ...
  wget https://raw.githubusercontent.com/saggerbr/chatwoot/master/deployment/nginx_chatwoot.conf
...
function cwctl_upgrade_check() {
    ...
    local remote_version_url="https://raw.githubusercontent.com/saggerbr/chatwoot/master/VERSION_CWCTL"
...
function upgrade_cwctl() {
    wget https://raw.githubusercontent.com/saggerbr/chatwoot/master/deployment/install.sh -O /usr/local/bin/cwctl > /dev/null 2>&1 && chmod +x /usr/local/bin/cwctl
}
